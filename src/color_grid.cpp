...
ColorGrid::ColorGrid(shared_ptr<TextLoader> text_loader, float view_width, float view_height) {
  this->text_loader = text_loader;

  int grid_size = text_loader->get_integer("IDS_COLORGRID_SIZE");
  ...
  for(int row=0;row<(((int)view_height)/grid_size);row++){
    grid.emplace_back(
            vector<sf::RectangleShape>(
                    (unsigned long)view_width/grid_size));
    ...
    for(int col=0;col<(((int)view_width)/grid_size);col++){
      grid[row][col]=sf::RectangleShape(sf::Vector2f(grid_size,grid_size));
      //Because row corresponds to ycor and col is xcor, we have to do this. A bit weird, I know.
      grid[row][col].setPosition(grid_size*col,grid_size*row);
...
void ColorGrid::update(float x, float y, int width, int height, sf::Color color) {
